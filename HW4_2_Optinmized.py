...
def cart(df, label, length, criteria, depth, number):
...
def random_forest(df, label, depth, number, criteria='gini'):
    total = df.shape[0]
    tree_array = []
    for i in depth:
...
def optimized_parameters(df, df_label, depth_list, number_list, n=5):
    best_depth, best_number = 0, 0
    best_fpr, best_tpr = 0, 0
    current_max = 0
    kfold = StratifiedKFold(n_splits=n, shuffle=True)
    depth_index = 0
    for train_index, validate_index in kfold.split(df, df_label):
        for number in number_list:
            tree_array = random_forest(df[train_index], df_label[train_index], depth_list[depth_index], number,
                                       criteria='gini')
            fpr, tpr, auc_value = get_roc_curve(df[validate_index], df_label[validate_index], tree_array)
            if auc_value > current_max:
                current_max = auc_value
                best_depth, best_number = depth_list[depth_index], number
...
df, df_label = np.array(df.drop(columns='label')), np.array(df.label)
